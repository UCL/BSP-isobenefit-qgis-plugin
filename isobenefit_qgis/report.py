...
def options_table(options: list[dict], target_population: float) -> list[str]:
    ...
    rows_spec = [
        ...
        ("served coverage, new homes (centre AND green)", lambda m, n: _pct(m.get("served_coverage"))),
        ("incl. existing fabric (no guarantee)", lambda m, n: _pct(m.get("served_coverage_incl_existing"))),
        ("existing homes alone (FYI)", lambda m, n: _pct(m.get("existing_served_coverage"))),
        ("mean walk, existing homes: centre (m)", lambda m, n: _num(_finite(m.get("existing_centre_walk_mean")))),
        ("mean walk, existing homes: green (m)", lambda m, n: _num(_finite(m.get("existing_green_walk_mean")))),
        ("centre coverage, new homes", lambda m, n: _pct(m.get("centre_coverage"))),
        ("green coverage, new homes", lambda m, n: _pct(m.get("green_coverage"))),
        ("unserved new homes", lambda m, n: _pct(m.get("unserved_fraction"))),
