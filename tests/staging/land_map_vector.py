...
def _green_to_built(
    ...
    granularity_m: int,
    max_distance_m: int,
    min_green_cont_km2: int | float = 0.25,
) -> tuple[bool, Any, Any]:
    """
    ...
    if not green_spans(state_arr, y_idx, x_idx, granularity_m, min_green_span_m=span_m):
    ...
    def iterate(self):
        ...
        for y_idx, x_idx in arr_idxs:
            ...
            if self.green_itx_arr[y_idx, x_idx] == 2:
                # don't allow double steps, i.e. a new built cell has to have at least one previously built neighbour
