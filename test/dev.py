...
def test_plot(land: land_map.Land):
    ...
    axes[1][0].imshow(land.cent_acc_arr, origin="lower")
    axes[1][0].set_title("access to centres")
    axes[1][1].imshow(land.green_acc_arr, origin="lower")
    axes[1][1].set_title("green access")
    axes[2][0].imshow(land.density_arr, origin="lower")
    axes[2][0].set_title("density")
    if land.areas_arr is not None:
        axes[2][1].imshow(land.areas_arr, origin="lower")
        axes[2][1].set_title("green s.m.")
    plt.tight_layout()
...
def test_land_map():
    ...
    granularity_m = 50
    walk_dist_m = 500
    min_long_green_span_m = 750
    min_short_green_span_m = 150
    # y is rows
