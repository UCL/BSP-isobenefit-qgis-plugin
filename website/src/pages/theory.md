...
| New centralities seed near built land that has outgrown its centres, with probability 0.005 | Released rather than drawn: each new home is charged to the centre it is nearest, and a centre earns its successor once that population reaches the service viability threshold, or once it can no longer grow. A seeding probability is uninterpretable to the person setting it and yields a different settlement count as window, cell size or growth rate change; people per centre is a planning judgement, and it is the figure the viability test already applies [Service viability] | Modified: earned, not drawn |
...
| One walkable radius T* = 5 cells, cell side 1000/T* m, so T* ≈ 1 km | Explicit metres throughout: cell size [Grid size] and four walking distances, defaulting to 1,200 m for centres, 400 m for green, and the 400 m stop and 1,200 m hub catchments that transit adds | Reparameterised |
