...
title: Theory
description: How the plugin's growth rules relate to D'Acci's published Isobenefit Urbanism model, rule by rule
---
...
The table lists each rule of the published simulator, what this implementation does,
and a status: **faithful** (same rule), **reparameterised** (same rule, different
units or defaults), **modified** (behaviour differs), or **extension** (no counterpart
in the published model). Parameter names in brackets are the plugin's dials.

| Published rule | This implementation | Status |
|---|---|---|
| Build only on nature cells adjacent to built land (periphery growth) | Same: a candidate must be empty and touch built land | Faithful |
| A centrality must be within T* of the new cell, measured as straight-line distance | A centre must be within the centre walk, measured as a bounded walk over the grid that cannot cross unbuildable land [Centre walk] | Modified: walks, not radii |
| Build with probability 0.5 once the checks pass | Same mechanism; the default is 0.25 [Build probability] | Reparameterised |
| Nature remains one connected region, and every nature strip stays at least T* wide | Local rules instead: no green corridor may be pinched below the minimum span, and a build that splits green must leave each fragment a minimum contiguous area [Min green span] | Modified: local, not global |
...
Two consequences follow from the table. First, the canonical behaviour is not fully
recoverable from the plugin's settings: the global nature-connectivity rule has no
switch, and the neighbouring-centrality probability is fixed. The plugin does not
claim otherwise. Second, most of the modifications share one motivation: the
published model assumes a uniform abstract plain, and real places arrive with
fragmented green, rivers, motorways and existing towns that the abstract rules cannot
accommodate unchanged.
...
scoring runs, and selecting the best. The boundary matters for interpretation: the
walkability guarantees are enforced by the growth rules during the run, while the
post-processing improves the presentation and the centre arrangement without adding
or removing population. The [recommended-plan notes](https://github.com/UCL/BSP-isobenefit-qgis-plugin/blob/main/docs/recommended-plan.md)
describe that pipeline step by step.
...
benefit wherever one lives, and is not a claim that the plugin's outputs achieve
perfect equality; the run report states the achieved coverage.
