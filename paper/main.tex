...
post-processing. The subsections below follow that sequence: the input
layers, the grid they become, the growth rules, the ensemble and the
selection of the best single run, and the post-processing that derives
the plan options.
...
user-selected cell size of 50~m or finer. Corridors along motorways,
trunk roads, railways, and rivers are marked unbuildable, so walking
routes detour around them and development cannot cross them. Other
street geometry is retained for display only.

Existing built fabric is frozen: the simulation leaves it unchanged,
treats its residents as served, and counts only new residents toward
the population target.

Slivers and pockets that cannot carry viable development are
eliminated from consideration: open land too narrow to build on, or in
a region too small to hold the minimum settlement, is reclassified as
protected green before the run and still counts toward green access.

Growth and scoring both measure distance with the bounded grid walk: a
shortest path over traversable cells, blocked by unbuildable land,
bounded at the maximum walking distance, and computed by Dijkstra
search. Street-network distances were tested in earlier releases and
removed. A new settlement's streets do not yet exist, so the metric
combined network distances over existing streets with assumed routes
over new fabric, and the two are not comparable.
...
expansion (Pajarito, Medell\'in). Three further scenarios (Dnipro,
Celina, and Kigali East)
