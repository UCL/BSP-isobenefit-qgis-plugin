...
Hill in London, and a hillside expansion at Pajarito in Medell\'in.
All scenario data, parameters, and code are public, and the
...
workplaces should be within a walk, and so should green space. The idea originates
...
model's term for centres) under a small set of rules,
...
the detailed form emerges \citep{dacci_banister_white2024}. Urban simulation models can become planning instruments, as
UrbanSim, an open land-use and transport model, did when it entered
use with metropolitan planning agencies \citep{waddell2002}. We
aim to operationalise Isobenefit Urbanism in the same way, by adapting
it to real-world constraints so that planners can use it to
conceptualise options and to consult residents and experts.
...
accommodate what is already there: towns, fragmented green space,
rivers, railways, motorways, and steep slopes.
Its outputs must also be in a GIS format that planners can work with.
We therefore add the inputs, the modelling constraints, and the
post-processing, and implement the result as a QGIS plugin.
...
\item An open, reproducible implementation: a QGIS plugin that wraps a
  simulation engine published on PyPI, with documentation and worked
  scenarios.
\item A plan-derivation pipeline on top of the automaton. An ensemble
  of runs yields likelihood layers, which show where development is
  robust across runs, and plan options derived from the best single
  run, with centres and densities arranged for walkable access.
\end{enumerate}
...
long line of cellular models of urban form \citep{white_engelen1993,
...
which keep nature as one connected region whose width never falls
...
applies from 2 hectares, which is the starting default for the
minimum park area (Section~\ref{sec:growth}). The 400~m starting default is slightly
...
layers from OpenStreetMap. This step is optional: users can supply
equivalent layers from their own data, or download the layers and
edit them. The simulation tool passes these inputs to the
...
cell takes a class from the layers that cover it: existing built
fabric, green space, unbuildable land, or open land. Corridors along
motorways, trunk roads, railways, and rivers are also marked
unbuildable. The remaining street geometry is kept for display
only.

The simulation models new development and holds existing fabric
fixed. Population densities for the existing fabric are generally
unavailable from OpenStreetMap, so the model assigns density tiers,
centres, and resident counts to new development only. The population
target therefore refers to new residents.

The simulation tool then screens the open land for viability. Slivers
too narrow to build on, and pockets too small to hold a settlement of
the configured minimum size, are reclassified as protected green
before the run; those that reach the minimum park area qualify as
parks (Section~\ref{sec:growth}).

In both the growth rules and the scoring, the simulation measures
distance with the bounded grid walk: a shortest path over traversable
cells, blocked by unbuildable land, bounded at the maximum walking
distance, and computed by Dijkstra search. Earlier releases
experimented with a hybrid measure that used street-network distances
over the existing fabric and grid walks over new growth, which has no
streets until a later design stage. We set the hybrid aside on
principle, because a distance measured partly over streets and partly
over the grid has no consistent meaning.
...
intact. A park is a contiguous green area of at least the minimum
park area, a parameter whose 2-hectare default follows Natural
England's accessible natural greenspace standard
\citep{naturalengland2010}. A
green corridor between developments may not narrow below the minimum
span, and new development may not shrink a park below the park area,
whether by splitting it or by building along its edge. New development
must also keep a park within the green walk of the new home and of
every built cell, the existing fabric included. Green areas smaller than a park
provide no green access and may be built over. A candidate
that passes every check builds at the
configured build probability, which staggers
...
draw and begin a new satellite settlement. Setting the probability to
zero disables leapfrogging. A run stops when it
reaches the population target.

\subsection{Ensembles and selection}
\label{sec:ensemble}

An ensemble consists of a configurable number of runs, set to fifty
in the scenarios reported here. Each run starts from the same inputs,
...
Because the layers average runs that disagree cell by cell, they do
not themselves satisfy the constraints and are informational
only. We therefore post-process each run into a candidate
plan and select the run whose new homes have the lowest mean walk to
a centre and to green as the final plan. Section~\ref{sec:fouroutputs}
describes the post-processing.

We also
report served coverage, the share of new homes within the chosen
walks of both a centre and a park. The growth rules guarantee both
walks for every new home, so the figure verifies that post-processing
preserved the guarantee. A companion figure blends in
the existing fabric, which carries no guarantee: the model assumes
existing homes are served by their own centres, and the growth rules
promise only that new development never removes an existing home's
park within the green walk. All measures use the bounded grid walk of the growth
rules, judging the centre and the green distance each against its own
walk, and a reported mean walk averages over the new homes within
that walk of the amenity.
...
is not viable development. The same rule applies to an outlying
hamlet: where a settlement of existing fabric is itself below the
...
studies below report that option.
...
order-independent integer sums. The test suite cross-checks the Rust
engine against a reference Python implementation of the walk metric
and the component labelling. A single verification script runs the full gate
...
expansion (Pajarito, Medell\'in). Each case's figure varies the setting
its constraints make most legible: the centre walk at Cambourne, the
density mix at Crews Hill, and dispersal at Pajarito; the centre-walk
sweep of Table~\ref{tab:sweep} covers all three. Three further
scenarios (Dnipro, Celina, and Kigali East)
...
accommodates about 16{,}300 of the 30{,}000 target. Every new home is
served, coverage including existing fabric is 89\%, and new homes
average a 212~m walk to a centre and 108~m to green. The window's developable land,
...
four outputs, which \path{scripts/demo_run_report.py} regenerates. The raw grown state credits 78\% of the target, and pruning reduces
this to 61\%. This window grows part of its population as detached
satellite clusters that stay below the 1{,}000-person minimum, and
pruning reverts them to nature. Every new home is served in all four
outputs; with the existing fabric blended in, coverage reads
94--99\%. Among the three plan options, which differ
only in centres, optimised placement cuts the mean centre walk from 233
to 179~m with the same eighteen centre areas as grown, and
fewest centres removes one centre area at the cost of part
of the reduction (204~m). Fewest centres starts from the optimised
...
target): the raw grown state and the three plan options. Coverage and walks
are measured over new homes; the incl.-existing row blends in the frozen
fabric, which carries no guarantee.}
...
\midrule
Population accommodated & 9{,}327 & 7{,}360 & 7{,}353 & 7{,}353 \\
Share of target & 78\% & 61\% & 61\% & 61\% \\
Built cells (incl.\ existing) & 1{,}962 & 1{,}685 & 1{,}685 & 1{,}685 \\
Mixed-use centre areas & 26 & 18 & 18 & 17 \\
Served coverage, new homes & 100\% & 100\% & 100\% & 100\% \\
Incl.\ existing fabric (no guarantee) & 99\% & 94\% & 94\% & 94\% \\
Mean walk to a centre (m) & 274 & 233 & 179 & 204 \\
Mean walk to green (m) & 104 & 109 & 109 & 109 \\
Centre \sqm{} per person & 5 & 19 & 19 & 19 \\
Green \sqm{} per person & 779 & 1{,}081 & 1{,}082 & 1{,}082 \\
\bottomrule
\end{tabular}
\end{table}
...
left), and 1{,}600~m (bottom right). Coverage including existing
fabric rises with the permitted walk. Full scenario resolution, seed 42.}
\label{fig:cambourne}
\end{figure}
...
At the baseline walk the plan reaches 12{,}936 of the 13{,}000 target;
coverage including existing fabric is 77\%, and new homes
average a 74~m walk to a centre and 76~m to green.
Figure~\ref{fig:crewshill} varies the density
mix on the fixed boundary. Shifting the shares one step toward the high
tier houses 1\% more people on 5\% fewer built cells; one step toward
the low tier takes 7\% more built cells for the same population.
Across the two directions, populations within 1\% of each other span a
13\% difference in land take within the release. The station anchor holds the main centre against the rail
...
recommended plan, with dispersal off, at three density mixes: the
baseline shares (top right), shares shifted one step toward the low
tier (bottom left), and one step toward the high tier (bottom right).
The same target takes visibly different amounts of the release. The
station anchor holds the main centre against the rail line in every
mix. Full scenario resolution, seed 42.}
\label{fig:crewshill}
\end{figure}
...
At the baseline walk the plan accommodates about 32{,}200 of the 40{,}000
target with coverage including existing fabric at 94\% and a 56~m mean
centre walk for new homes
(Figure~\ref{fig:medellin}), with the steep-slope mask fragmenting the
buildable land. Figure~\ref{fig:medellin} varies dispersal, and
the moderate baseline does best of the three settings. Turning
dispersal off lowers the accommodated population (22{,}900 people, with
coverage including existing fabric at 93\%), because contiguous growth
cannot reach the buildable
benches that only satellite seeding can start. Aggressive dispersal
also underperforms the baseline (27{,}700 people at 91\%):
some dispersal is needed to reach the benches, and past that point
further scatter fragments the growth without housing more people.
...
\includegraphics[width=0.48\textwidth]{figures/medellin_pajarito_compact.png}\hfill
\includegraphics[width=0.48\textwidth]{figures/medellin_pajarito_dispersed.png}\\[6pt]
\includegraphics[width=\textwidth]{figures/legend.png}
\caption{The Pajarito window before growth (top left) and the recommended
plan under three dispersal settings: the moderate baseline (top right),
dispersal off (bottom left), and aggressive (bottom right). Slopes over
20\(^\circ\) are unbuildable and fragment the buildable land into
benches that contiguous growth cannot reach. Full scenario resolution,
seed 42.}
\label{fig:medellin}
\end{figure}
...
mean walk and raises coverage including existing fabric in every
case. The rise is largest at Pajarito between the 400 and 800~m
settings, because many existing hillside homes sit within 800~m of a
centre but beyond 400~m. Each figure comes from one selected run.

\begin{table}[t]
\centering
\caption{Coverage including existing fabric, and the mean centre walk of
new homes, under three centre-walk settings. Every new home is served by
construction; the existing fabric carries no guarantee.}
\label{tab:sweep}
\small
...
\midrule
Cambourne & 78\% & 159~m & 89\% & 212~m & 99\% & 280~m \\
Crews Hill & 63\% & 47~m & 77\% & 74~m & 91\% & 88~m \\
Pajarito & 67\% & 44~m & 94\% & 56~m & 97\% & 71~m \\
\bottomrule
\end{tabular}
\end{table}
...
The gap between the raw grown state and the plan options (78\% against 61\%
...
threshold that removes about a fifth
...
Served coverage over new homes holds by construction, so it verifies
the pipeline rather than discriminating between settings. Coverage
including existing fabric counts every home in the window, so it does
not compare across windows with different shares of existing fabric,
and within-scenario comparisons are the intended reading. Every reported
...
settings, such as mean walks a few metres apart. The pipeline has
