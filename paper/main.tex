...
the detailed form emerges spontaneously \citep{dacci_banister_white2024}. Urban simulation models can become useful planning instruments, as
UrbanSim, an open model of land use and transport, did when it entered
use with metropolitan planning agencies \citep{waddell2002}. We
likewise aim to operationalise Isobenefit Urbanism by adapting it to
real-world constraints, so that planners can use it as part of the
planning process, to conceptualise options and to engage in dialogue
and feedback with residents and experts in their respective fields.
...
rules easy to study. In real-world use, however, the model must
accommodate existing constraints: towns, fragmented green space,
...
is set to 0.5, for example, an eligible cell has an even chance of
building at each step. New
...
knowledge, and bands above the scenario's slope limit count as
unbuildable.
...
distance, and computed by Dijkstra search. In earlier releases we experimented with a hybrid measure,
street-network distances over the existing fabric joined to grid
walks over new growth, which has no streets until a later design
stage. We decided on principle not to proceed, because a distance
measured partly over streets and partly over the grid has no
consistent meaning.
...
are not counted for green access and may be built over. A candidate
...
satellite settlement around it. The dispersal probability can be set
to zero where leapfrogging is undesirable. A run stops when it
...
The likelihood layers are for information only. Because they average
runs that disagree cell by cell, the layers do not themselves satisfy
the constraints. We therefore post-process each run into a candidate
...
walks for every new home, so this figure doubles as a check that
post-processing preserved the guarantee. A companion figure blends in
