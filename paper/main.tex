...
available land. The outcome ranges from regrowth wholly inside the
district boundaries to no growth, so the outputs depend strongly on which
land counts as available. All scenario data, parameters, and code are public, and the
...
eligible to become built if a centrality lies within the walkable
radius T\(^*\), typically set to about one kilometre or a
fifteen-minute walk, and the change would not break the nature rules. Those rules
keep nature as one continuously connected region that retains a minimum
...
attached to the existing fabric. A candidate must hold a centre within
the centre walk, which keeps new homes within walkable reach of shops
and services. Building it must also leave the green network
...
Centres follow the same logic. When built land outgrows the service of
its existing centres, a new centre may seed nearby, again by a random
draw at a configurable seeding probability, with at most one new
centre per iteration, and seeding stops once the run reaches 80\% of
the population target. With dispersal enabled, isolated centres also
seed on open land by a per-cell draw, at a rate the dispersal setting
controls, and begin new satellite settlements. A run stops when it
...
is post-processing on that state. A stochastic run can leave stranded
fragments, clusters of new growth below the minimum settlement size
that are too small to function as settlements or to support a centre of
their own. Post-processing therefore applies two steps before the
centre options: a cleanup of these fragments and a density
arrangement.

The cleanup treats a sub-threshold cluster according to its position.
A detached cluster is pruned and reverts to nature, because it is not
viable development. A cluster grown against existing fabric is instead
absorbed into that fabric, because the odd free cell inside a town is
usually a mapping artefact, such as an unmapped road, a park, or an
irregular plot, rather than developable land. Absorption adds no
population; the cells simply drop out of the population, density, and
centre accounting. The cleanup is the only step that changes the
population, so a plan option can carry less than the raw state.
Existing fabric is never pruned.
...
wherever new development lacks provision within the walk; existing centres
serve only the existing fabric. \emph{Fewest centres} additionally
...
accommodates about 10{,}100 of the 30{,}000 target with 70\% of all homes
served (within the walk of both a centre and green), a mean centre walk
of 366~m, and a mean green walk of 95~m.
...
four outputs. The raw grown state credits 82\% of the target, and the
post-processing steps reduce this to 51\%. This window grows much of its
population as satellite clusters below the 1{,}000-person minimum, which
pruning reverts and absorption folds into existing fabric. Served
coverage falls from the raw 95\% to 87--88\% in the plan options because
the metric counts every home in the window, including existing fabric,
against both walks, while the enforced constraint binds only new homes and
only the centre walk. Among the three plan options, which differ only
in centres, optimised placement cuts the mean
centre walk from 482 to 436~m with three more centres than as grown (18
against 15), and fewest centres keeps most of that reduction (442~m) with
two fewer centres. Fewest centres starts from the optimised placement and
removes centres only while every home keeps one within the walk, so its
count of 16 can exceed the as-grown 15.
...
Mean walk to a centre (m) & 292 & 482 & 436 & 442 \\
Mean walk to green (m) & 145 & 130 & 130 & 130 \\
Centre \sqm{} per person & 5 & 17 & 20 & 19 \\
Green \sqm{} per person & 867 & 1{,}500 & 1{,}500 & 1{,}500 \\
\bottomrule
\end{tabular}
\end{table}
...
At the baseline walk the plan accommodates about 9{,}500 of the 13{,}000
target with 71\% of homes served, a mean centre walk of 284~m, and a mean
green walk of 110~m (Figure~\ref{fig:crewshill}). Shifting the shares
one step toward the high tier houses 8\% fewer people on 6\% fewer built
cells, so within the release boundary the denser preset
saves land at a population cost. The station anchor holds the main centre against the rail
...
At the baseline walk the plan accommodates about 23{,}900 of the 40{,}000
target with 44\% of homes served and a mean centre walk of 164~m
...
raises coverage to 49\% and the accommodated population to 32{,}100,
since contiguous growth wastes less of the scarce buildable land on
satellite formation.
...
1{,}600~m for the three cases. A longer permitted walk raises served
coverage and lengthens the mean walk in every case. Cambourne, an open
window with little existing fabric, reaches 94\% coverage at 1{,}600~m,
while Pajarito moves least because the terrain mask binds the form before
the centre walk does. In Cambourne the accommodated population falls as
the walk lengthens (15{,}226 at 400~m against 5{,}643 at 1{,}600~m), so
coverage and delivered population trade off in this window. Windows
dominated by existing fabric sit lower on this metric, which counts every
home in the window (Section~\ref{sec:discussion}).
...
\midrule
Cambourne & 56\% & 173~m & 70\% & 366~m & 94\% & 700~m \\
Crews Hill & 52\% & 165~m & 71\% & 284~m & 86\% & 426~m \\
Pajarito & 38\% & 59~m & 44\% & 164~m & 52\% & 332~m \\
\bottomrule
\end{tabular}
\end{table}
...
patch preceded the containment result reported below, so the patched
layer contributes to that containment. Regional-plan set-asides may not
...
Under the pre-plan reading, the model regrows 1{,}216 cells housing about
10{,}900 residents, and all of that growth falls inside the district
boundaries. The shortfall against the 16{,}000 target is roughly Vauban's
share, which the green-span rule blocks (below). The containment stops
short of footprint agreement, because
the cell-level overlap with the real footprint is weak (intersection over
union 0.10) and the build likelihood sits higher on the land the
planners set aside for the reserve (0.20) than on the land they built
(0.14). The mean walks land close to the real districts' (376~m
against 397~m to a centre, 191~m against 171~m to green). This agreement
is a consistency check rather than evidence of fidelity, because the
scored output is optimised placement, whose objective minimises the mean
centre walk under the 800~m cap, and run selection keeps the
lowest-mean-walk run.

The containment follows from two growth rules: growth must touch existing
fabric and keep a centre within the 800~m walk. At the start only 15 of
the Rieselfeld field's roughly 4{,}400 growable cells qualify, and they
all lie beside the one existing centre on the field's eastern edge. Every
run therefore enters the field where the real district also attaches to
the town. Vauban's land is open in the inputs (446 growable cells, an
existing centre 71~m from its edge), and every run grows exactly nine
cells there and stops, because the minimum green span admits no more. The
district's open strip is narrower than 400~m, so any further build would
pinch the green corridor below the minimum. The nine cells fall below the
1{,}000-person minimum settlement and are absorbed into the existing
fabric, so the recommended plan shows none. The real Vauban sits outside
what the growth rules permit at their default green span. Halving the
span to 200~m lifts the regrowth to 124 raw grown cells before pruning
and absorption.
...
intersection over union of 0.15 at 200~m to 0.07 at 600~m. The real
...
default 400~m. Vauban's plan-level regrowth stays at zero across the
sweep, because its capacity sits at the minimum-settlement threshold.
...
\midrule
New cells & 1{,}062 & 1{,}216 & 1{,}144 \\
Population & 9{,}558 & 10{,}944 & 10{,}296 \\
Growth inside the district boundaries & 99\% & 100\% & 100\% \\
Footprint overlap (intersection over union) & 0.15 & 0.10 & 0.07 \\
Mean walk to a centre (m) & 374 & 376 & 380 \\
Mean walk to green (m) & 197 & 191 & 183 \\
Real districts' served coverage at this span & 54\% & 49\% & 37\% \\
\bottomrule
\end{tabular}
\end{table}

Under the present-day reading, no plan-level growth survives. Raw growth
is nearly absent, with a mean build likelihood of 0.005 on the real
district footprint, so a few cells build in a few runs and
post-processing removes them. Present-day green designations, the carved
rail corridors, and the withheld land leave almost no admissible parcel
reachable from the existing fabric. The districts could not be regrown
under the designations that now surround them.
...
\midrule
Built cells (district / new) & 1{,}929 & 1{,}216 & 0 \\
Population & 16{,}000 & 10{,}944 & 0 \\
Mean walk to a centre (m) & 397 & 376 & --- \\
Mean walk to green (m) & 171 & 191 & --- \\
Growth inside the district boundaries & --- & 100\% & --- \\
Footprint overlap (intersection over union) & --- & 0.10 & --- \\
Mean build likelihood on the real footprint & --- & 0.14 & 0.005 \\
\bottomrule
\end{tabular}
\end{table}
...
The gap between the raw grown state and the plan options (82\% against 51\%
of target in the Cambourne demonstration window) is entirely produced by
the minimum-settlement rules. The pruned clusters fall below the
configured 1{,}000-person minimum, and keeping them as free-standing
settlements would overstate coverage. At the same time, a threshold that removes 38\%
...
settings, such as footprint overlaps of 0.15 against 0.10 or mean walks a
