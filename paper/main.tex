...
available land. The outcome ranges from regrowth concentrated inside
the district boundaries to growth that avoids them almost entirely, so
the outputs depend strongly on which land counts as available. All scenario data, parameters, and code are public, and the
...
Preparation also sets aside buildable pockets. Growth is contiguous,
so a new settlement can never outgrow the connected patch of buildable
land it starts in, and a patch smaller than the minimum settlement
size therefore cannot host viable development. Such a patch is
reclassified as protected green, which keeps it walkable and counted
as green space. In open land this rules out doomed satellites, and a
small patch enclosed by a town becomes a pocket park instead of
receiving piecemeal infill.
...
is not viable development. A cluster grown against existing fabric is
kept whatever its size, because its growth was anchored on the town's
centres and counts as legitimate infill; existing fabric itself is
also never pruned. Pruning is the only step that changes the
...
Buildable pockets smaller than the minimum settlement are reclassified
as protected green before the run; a pocket enclosed by a town becomes
a pocket park &
Extension \\
\addlinespace
...
accommodates about 21{,}800 of the 30{,}000 target with 98\% of all homes
served (within the walk of both a centre and green), a mean centre walk
of 222~m, and a mean green walk of 94~m.
...
four outputs, which \path{scripts/demo_run_report.py} regenerates. The raw grown state credits 82\% of the target, and pruning reduces
this to 59\%. This window grows part of its population as detached
satellite clusters that stay below the 1{,}000-person minimum, and
pruning reverts them to nature. Served coverage holds at 96--98\%
across all four outputs. Among the three plan options, which differ
only in centres, optimised placement cuts the mean centre walk from 255
to 210~m with eight more centre areas than as grown (28 against 20),
and fewest centres keeps most of that reduction (216~m) with one fewer.
Fewest centres starts from the optimised placement and removes centres
only while every home keeps one within the walk, so its count of 27 can
exceed the as-grown 20.
...
\midrule
Population accommodated & 9{,}897 & 7{,}082 & 7{,}068 & 7{,}068 \\
Share of target & 82\% & 59\% & 59\% & 59\% \\
Built cells (incl.\ existing) & 2{,}042 & 1{,}643 & 1{,}643 & 1{,}643 \\
Mixed-use centre areas & 27 & 20 & 28 & 27 \\
Served coverage (centre and green) & 96\% & 97\% & 98\% & 98\% \\
Mean walk to a centre (m) & 276 & 255 & 210 & 216 \\
Mean walk to green (m) & 115 & 115 & 115 & 115 \\
Centre \sqm{} per person & 5 & 19 & 22 & 22 \\
Green \sqm{} per person & 814 & 1{,}279 & 1{,}281 & 1{,}281 \\
\bottomrule
\end{tabular}
\end{table}
...
At the baseline walk the plan accommodates about 13{,}000 residents,
effectively meeting the 13{,}000
target with 76\% of homes served, a mean centre walk of 241~m, and a mean
green walk of 108~m (Figure~\ref{fig:crewshill}). Shifting the shares
one step toward the high tier houses 3\% more people on 5\% fewer built
...
At the baseline walk the plan accommodates about 31{,}500 of the 40{,}000
target with 73\% of homes served and a mean centre walk of 167~m
...
raises the accommodated population to about 40{,}600 and lowers
coverage to 62\%, because the denser contiguous fabric pushes more
homes beyond the green walk.
...
coverage and lengthens the mean walk in every case. The effect is
largest at Crews Hill, where coverage runs from 64\% at 400~m to 90\%
at 1{,}600~m, and smallest at Pajarito, where the terrain mask binds
the form before the centre walk does. Windows dominated by existing
fabric sit lower on the coverage metric, which counts every home in
the window (Section~\ref{sec:discussion}).
...
\midrule
Cambourne & 91\% & 157~m & 98\% & 222~m & 99\% & 260~m \\
Crews Hill & 64\% & 132~m & 76\% & 241~m & 90\% & 408~m \\
Pajarito & 68\% & 104~m & 73\% & 167~m & 73\% & 224~m \\
\bottomrule
\end{tabular}
\end{table}
...
Under the pre-plan reading, the model regrows 1{,}681 cells housing about
15{,}100 residents, and 87\% of that growth falls inside the district
boundaries, with Rieselfeld taking 1{,}457 cells. The concentration
stops short of footprint agreement, because
the cell-level overlap with the real footprint is weak (intersection over
union 0.11) and the build likelihood sits higher on the land the
planners set aside for the reserve (0.21) than on the land they built
(0.15). The regrown plan's mean walks are shorter than the real
districts' (360~m against 397~m to a centre), which follows from the
...
The concentration follows from two growth rules: growth must touch existing
...
the town, and the growth outside the boundaries is contiguous extension
of the existing fabric elsewhere in the window, since dispersal is off
in this scenario. Vauban's land is open in the inputs (446 growable
cells, an existing centre 71~m from its edge), and every run grows
exactly nine cells there and stops, because the minimum green span
admits no more. The
district's open strip is narrower than 400~m, so any further build would
pinch the green corridor below the minimum. The nine cells stay in the
plan as infill against the existing fabric, and the real Vauban sits
outside what the growth rules permit at their default green span.
Halving the span to 200~m lifts the regrowth there to 124 cells.
...
intersection over union of 0.15 at 200~m to 0.09 at 600~m. The real
...
default 400~m. Vauban's regrowth holds at nine cells at the 400 and
600~m spans and reaches 124 cells only at 200~m, because the green span
is what bounds it.
...
\midrule
New cells & 1{,}608 & 1{,}681 & 1{,}256 \\
Population & 14{,}472 & 15{,}129 & 11{,}304 \\
Growth inside the district boundaries & 83\% & 87\% & 90\% \\
Footprint overlap (intersection over union) & 0.15 & 0.11 & 0.09 \\
Mean walk to a centre (m) & 360 & 360 & 366 \\
Mean walk to green (m) & 189 & 190 & 179 \\
Real districts' served coverage at this span & 54\% & 49\% & 37\% \\
\bottomrule
\end{tabular}
\end{table}

Under the present-day reading, the districts stay almost untouched. The
mean build likelihood on their real footprint is 0.003, and only 4\% of
the regrown plan's growth falls inside the boundaries, because
present-day green designations, the carved rail corridors, and the
withheld land leave almost no admissible parcel inside the districts
reachable from the existing fabric. The model instead places about
2{,}000 residents against the existing fabric elsewhere in the window.
The districts could not be regrown under the designations that now
surround them.
...
\midrule
Built cells (district / new) & 1{,}929 & 1{,}681 & 223 \\
Population & 16{,}000 & 15{,}129 & 2{,}007 \\
Mean walk to a centre (m) & 397 & 360 & 380 \\
Mean walk to green (m) & 171 & 190 & 196 \\
Growth inside the district boundaries & --- & 87\% & 4\% \\
Footprint overlap (intersection over union) & --- & 0.11 & 0.002 \\
Mean build likelihood on the real footprint & --- & 0.15 & 0.003 \\
\bottomrule
\end{tabular}
\end{table}
...
(bottom left), and 600~m (bottom right). Regrown development
concentrates inside the district boundaries (83\% at 200~m, 87\% at
400~m, 90\% at 600~m). Optimised placement, best of fifty runs, 25~m grid.}
\label{fig:freiburg}
\end{figure}
...
The gap between the raw grown state and the plan options (82\% against 59\%
...
settings, such as footprint overlaps of 0.15 against 0.11 or mean walks a
